...
RUN --mount=type=cache,id=cacheKey-npm-frontend,target=/root/.npm npm ci
...
RUN --mount=type=cache,id=cacheKey-npm-api,target=/root/.npm npm ci
...
# runtime image.
RUN --mount=type=cache,id=cacheKey-npm-api,target=/root/.npm npm prune --omit=dev
...
RUN --mount=type=cache,id=cacheKey-apt-cache,target=/var/cache/apt,sharing=locked \
    --mount=type=cache,id=cacheKey-apt-lib,target=/var/lib/apt,sharing=locked \
