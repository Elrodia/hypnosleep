...
RUN --mount=type=cache,id=s/d0c8496a-5b0f-472a-8b19-67dabc82f23d-npm-frontend,target=/root/.npm npm ci
...
RUN --mount=type=cache,id=s/d0c8496a-5b0f-472a-8b19-67dabc82f23d-npm-api,target=/root/.npm npm ci
...
# runtime image.
RUN --mount=type=cache,id=s/d0c8496a-5b0f-472a-8b19-67dabc82f23d-npm-api,target=/root/.npm npm prune --omit=dev
...
RUN --mount=type=cache,id=s/d0c8496a-5b0f-472a-8b19-67dabc82f23d-apt-cache,target=/var/cache/apt,sharing=locked \
    --mount=type=cache,id=s/d0c8496a-5b0f-472a-8b19-67dabc82f23d-apt-lib,target=/var/lib/apt,sharing=locked \
...
# Pre-deploy tooling inputs: drizzle configs + TypeScript source.
# Needed by the Railway `preDeployCommand` (see ../../railway.toml)
# which runs `drizzle-kit migrate` (reads the config files + SQL
# migration folders under `src/db/*/migrations`) and
# `tsx scripts/seed-templates` (imports from `src/*`) before the API
# is promoted to serving traffic.
